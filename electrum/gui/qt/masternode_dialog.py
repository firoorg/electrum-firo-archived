...
from electrum_xzc.masternode import MasternodeAnnounce
from electrum_xzc.masternode_manager import parse_masternode_conf
from electrum_xzc.protx_list import MNList
...
#        self.tabs.addTab(self.create_vote_tab(), _('Vote'))
        ...
        vbox = QVBoxLayout()
        self.dip3_warn = QLabel(_('Warning: DIP3 znodes is active, '
        ...
        vbox.addWidget(QLabel(_('Znodes:')))
    ...
    def create_masternode_conf_tab(self):
        ...
        import_filename_edit.setPlaceholderText(_('Enter the path to your znode.conf'))
    ...
    def import_masternode_conf(self, filename):
        ...
        if self.manager.wallet.has_password():
            pw = self.gui.password_dialog(msg=_('Please enter your password to import Znode information.'))
            if pw is None:
                return
        ...
        noun = 'znode' if num == 1 else 'znodes'
    ...
    def sign_announce(self, alias):
        ...
        util.WaitingDialog(self, _('Signing Znode Announce...'), sign_thread, on_sign_successful, on_sign_error)
    ...
    def send_announce(self, alias):
        ...
        def on_send_successful(result):
            ...
            if errmsg:
                self.logger.error(f'Failed to broadcast ZnodeAnnounce: '
                                  f'{errmsg}')
                self.show_critical(errmsg, title=_('Error Sending'))
            elif was_announced:
                self.logger.info(f'Successfully broadcasted '
                                 f'ZnodeAnnounce for "{alias}"')
                self.show_message(_('Znode activated successfully.'),
                                  title=_('Success'))
            self.masternodes_widget.refresh_items()
            self.masternodes_widget.select_masternode(alias)

        def on_send_error(err):
            self.logger.error('Error sending Znode Announce message:')
            # Print traceback information to error log.
            self.logger.error(''.join(traceback.format_tb(err[2])))
            self.logger.error(''.join(traceback.format_exception_only(err[0],
                                                                      err[1])))
        ...
        self.logger.info('Sending Znode Announce message...')
    ...
    def cast_vote(self, proposal_name, vote_yes):
        ...
        mn = self.selected_masternode()
        if not mn.announced:
            return self.show_critical(_('Znode has not been activated.'),
                                      title=_('Cannot Vote'))
